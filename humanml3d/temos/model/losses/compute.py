...
class TemosComputeLosses(Module):
    def __init__(self, vae: bool,
        ...
        mode = "human3dml" # Hardcoded: check later
        # Save parameters
        self.vae = vae
        self.mode = mode
        ...
        if mode == "smpl" or mode == "human3dml":
            if not ablation_no_motionencoder:
        ...
        if mode == "smpl_foot":
            ...
            losses.append("recons_text2rfeats")
            #contact losses
            losses.append("regression_contact")
    ...
    def update(self, ds_text=None, ds_motion=None, ds_ref=None,
        ...
        if self.mode == "human3dml":
            if not self.ablation_no_motionencoder:
                total += self._update_loss("recons_rfeats2rfeats", ds_motion.joints, ds_ref.joints)
            total += self._update_loss("recons_text2rfeats", ds_text.joints, ds_ref.joints)

        if self.mode == "smpl_foot":
            if not self.ablation_no_motionencoder:
                total += self._update_loss("recons_rfeats2rfeats", ds_motion.rfeats, ds_ref.rfeats)
            total += self._update_loss("recons_text2rfeats", ds_text.rfeats, ds_ref.rfeats)
            total += self._update_loss("regression_contact",(ds_text.contacts,ds_text.joints),ds_ref.contacts)
            total += self._update_loss("regression_contact",(ds_motion.contacts,ds_motion.joints),ds_ref.contacts)
