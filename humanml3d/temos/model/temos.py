...
class TEMOS(BaseModel):
    def __init__(self, textencoder: DictConfig,
        ...
        self.motionencoder = instantiate(motionencoder, nfeats=263) # Hardcoded: check later

        self.transforms = instantiate(transforms)
        self.Datastruct = self.transforms.Datastruct


        self.motiondecoder = instantiate(motiondecoder, nfeats=135) # Hardcoded: check later
    ...
    def motion_to_motion_forward(self, datastruct,
        ...
        if self.motiondecoder.contact:
            
            features = datastruct.rfeats.float()
        else:
            features = datastruct.features
