...
def evaluate(cfg: DictConfig) -> None:
    ...
    load_human3dml = False # Hard coded, check later
    ...
    from temos.data.human3dml import load_mmm_keyid, load_amass_keyid, load_human3dml_keyid
    ...
    if load_human3dml:
        datapath = Path(cfg.path.datasets) / "Human3DML"
    if amass:
    ...
    if load_human3dml:
        ...
        keyids = get_split_keyids(datapath, split)
    else:
    ...
    for keyid in keyids:
        ...
        # load mmm
        if load_human3dml:
            ref_joints, ref_features, success = load_human3dml_keyid(keyid, datapath)
        ...
        for index in range(cfg.number_of_samples):
            ...
            try:
                ...
                logger.info(f"Problem with '{keyid}' item")
                continue
