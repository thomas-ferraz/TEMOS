...
class KIT(Dataset):
    ...
    def __init__(self, datapath: str,
                 ...
                 load_with_vertices = False,
                 downsample=True,
                 tiny: bool = False, **kwargs):

        self.split = split
        self.load_amass_data = load_amass_data
        self.load_with_vertices = load_with_vertices
        ...
        for i, keyid in enumerator:
            ...
            # read smpl params
            if load_amass_data:
                if load_contact_data or load_with_vertices:
                ...
                if load_contact_data or load_with_vertices: 
                    ...
                    smpl_data, duration = downsample_amass_contact(smpl_data,framerate=framerate,with_vertices=load_with_vertices)
                    if load_contact_data:
                        contacts_data[keyid] = smpl_data["contacts"]
                    if load_with_vertices:
                        vertices_data[keyid] = smpl_data["vertices"]
    ...
    def _load_datastruct(self, keyid, frame_ix=None):
        ...
        if self.load_with_vertices:
            datastruct.set_vertices(self.vertices_data[keyid])
...
def downsample_amass_contact(smpl_data_,framerate, with_vertices=False,downsample=True):
    nframes_total = smpl_data_["contacts"].shape[0]

    last_framerate = smpl_data_["fps"].item()
    ...
    a = torch.from_numpy(smpl_data_["root_orient"])
    b = torch.from_numpy(smpl_data_["pose_body"])
    c = torch.cat((a,b),axis=1)
    smpl_data = {"poses": (c[frames]).float(),
                "trans": torch.from_numpy(smpl_data_["trans"][frames]).float(),
                "contacts":torch.from_numpy(smpl_data_["contacts"][frames]).float()[:,CONTACT_INDS]} # reducing to values of interest
    if with_vertices:
        smpl_data["vertices"] = torch.from_numpy(smpl_data_["mojo_verts"][frames]).float()
    return smpl_data,duration
