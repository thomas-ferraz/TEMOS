...
class SMPLFootTransform(Transform):
    ...
    def Datastruct(self, **kwargs):
        return SMPLFootDatastruct(_rots2rfeats=self.rots2rfeats,
...
@dataclass
class SMPLFootDatastruct(Datastruct):
    transforms: SMPLFootTransform
    _rots2rfeats: Rots2Rfeats
    _rots2joints: Rots2Joints
    _joints2jfeats: Joints2Jfeats

    features: Optional[Tensor] = (None,None)
    ...
    contacts_: Optional[Tensor] = None
    vertices_: Optional[Tensor] = None

    def __post_init__(self):
        self.datakeys = ["features", "rots_", "rfeats_",
                         "joints_", "jfeats_","contacts_","vertices_"]
        # starting point
        if self.features is not None and self.rfeats_ is None:
            self.contacts_ = self.features[...,-4:]
            self.rfeats_ = self.features[...,:-4]
